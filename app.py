...
def main():
    ...
    if choice == "Home":
        ...
        # Add search bar for college names
        search_college = st.text_input("Search CV by college name", key="search_college")
        if search_college:
            college_filtered_cvs = [cv for cv in st.session_state.cvs if any(search_college.lower() in text.lower() for label, text in cv['output'] if label == "ORG")]
            filtered_cvs = college_filtered_cvs
